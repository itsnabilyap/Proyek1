<!DOCTYPE html>
<html lang="en">
<head>
    <title>Detail Pesanan</title>
    <link rel="stylesheet" type="text/css" href="detailpesanan.css">
</head>
<body>

    <header class="navbar">
            <div class="logo">
                <img src="logo-brand.jpeg" alt="C2vin Logo">
                <h2>C2VIN</h2>
            </div>
            <ul class="navigasi">
                <li><a href="index.php#home">Home</a></li>
                <li><a href="index.php#about">About</a></li>
                <li><a href="menu.php">Menu</a></li>
                <li><a href="index.php#contact">Contact</a></li>
            </ul>
            <div class="navbar-extra">
                <a href="#" id="shopping-cart"><i data-feather="shopping-cart"></i></a>
            </div>
        </header>

        <main class="container content">
            <h1>Detail Menu</h1>
            <div class="product-detail">
                <div class="product-image">
                    <img src="AmericanBreakfast.jpeg" alt="tumpeng">
                </div>
                <div class="product-info">
                    <h1>American Breakfast</h1>
                    <p>Rp. 25.000</p>

                    <div class="form-group">
                        <label>/Pack</label>
                        <select>
                            <option value="">Pilih Pack</option>
                            <option value="Kecil">20 pack</option>
                            <option value="Sedang">30 pack</option>
                            <option value="Besar">50 pack</option>
                        </select>
                    </div>

                    <button class="button">Tambahan Pesanan</button>
                </div>
            </div>
            <div class="description">
                <h3>Deskripsi</h3>
            </div>
        </main>

        <footer class="footer" id="contact"> 
        <div class="footer-content">
            <div class="footer-box">
                <h3>Navigation</h3>
                <p>Home<br>
                About<br>
                Menu<br>
                Contact</p>
            </div>
            <div class="footer-box">
                <h3>C2VIN Catering</h3>
                <p>Jl. Letnan Sutejo, No.74<br>
                Kelurahan Margadadi,<br>
                Indramayu</p>
            </div> 
            <div class="footer-box">
                <h3>Contact WhatsApp</h3>
                <p>+62 877-2753-1916 (Liska)<br>
                +62 878-2691-3182 (Azwar)</p>
            </div>
        </div>
    </footer>

    <div class="copyright">
        Copyright © c2vin Catering, 2026. All rights reserved.
    </div>
        
</body>
</html>